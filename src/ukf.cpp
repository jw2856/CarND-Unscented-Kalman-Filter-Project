...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  for (int i = 0; i < 2*n_aug_+1; i++) {
    ...
    Zsig(2, i) = (p_x*v1 + p_y*v2)/std::max(0.001, rho);      //r_dot
  }
  ...
  // Combine the calculations of Tc and S
  for (int i=0; i<2*n_aug_+1; i++)
  {
    ...
    Tc = Tc + weights_(i)*x_diff*z_diff_transpose;
    S = S + weights_(i)*z_diff*z_diff_transpose;
  }
